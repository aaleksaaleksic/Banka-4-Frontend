...
export default function TransferForm({ accounts, onSubmit }: TransferFormProps) {
    const form = useForm({
    ...
    const filteredToAccounts = accounts.filter(
        (acc) => acc.currency.code === selectedFromAccount?.currency.code && acc.id !== fromAccount
    );


    function handleSubmit(data: { fromAccount: string; toAccount: string; amount: string }) {
        onSubmit({
                ...
                <CardContent>
                    <Form {...form}>
                        <form onSubmit={form.handleSubmit(handleSubmit)} className="space-y-6">
                                    ...
                                    <FormItem>
                                        ...
                                        {selectedFromAccount && (
                                            <p className="text-sm text-gray-600 mt-1">
                                                Available: <span className="font-semibold">{selectedFromAccount.availableBalance}</span>{' '}
                                            ...
                                            <Select onValueChange={field.onChange} disabled={!fromAccount}>
                                                ...
                                                <SelectContent>
                                                    {filteredToAccounts.length > 0 ? (
                                                        ...
                                                        <p className="text-gray-500 p-2 text-sm">No available accounts</p>
                                                    )}
                                                </SelectContent>
                                            </Select>
                                        </FormControl>
                          ...
                          {selectedFromAccount ? selectedFromAccount.currency.code : 'RSD'}
                        </span>
                                            </div>
                                        </FormControl>
                                        <FormMessage />
